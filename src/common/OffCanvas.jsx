...
const OffCanvas = () => {
    const { isOffCanvasOpen, handleOffCanvasClose } = useContext(OffCanvasContext);  
    const { handleScrollHideLgClose } = useContext(ScrollHideContext); 

    return (
        <>
            <div className={`side-overlay ${isOffCanvasOpen ? 'show' : ''}`} onClick={() => { handleOffCanvasClose(); handleScrollHideLgClose(); }}></div>
            {/* ==================== Right Offcanvas Start Here ==================== */}
            <div className={`common-offcanvas d-lg-block d-none ${isOffCanvasOpen ? 'active' : ''}`}>
